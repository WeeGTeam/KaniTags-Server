...
pub trait CollectionDatabase {
    ...
    fn load_collection_by_user_and_name(&self, user: &User, collection_name: &str) -> Result<Option<Collection>, anyhow::Error>;
    fn load_collections_by_user(&self, user: &User) -> Result<Vec<Collection>, anyhow::Error>;
    fn create_collection(&self, user: &User, name: &str) -> Result<Collection, anyhow::Error>;

...
impl CollectionService for CollectionServiceImpl {
    fn load_collections_by_user(&self, user: &User) -> Result<Vec<Collection>, LoadCollectionsError> {
        ...
        let collections = self.database.collection().load_collections_by_user(user)?;
    ...
    fn create_collection(&self, user: &User, collection_name: &CollectionName) -> Result<Collection, CreateCollectionError> {
        ...
        if let Some(existing_collection) = self.database.collection().load_collection_by_user_and_name(user, &collection_name)? {
        ...
        let collection = self.database.collection().create_collection(user, collection_name)?;
    ...
    fn delete_collection(&self, user: &User, collection_id: CollectionId) -> Result<(), DeleteCollectionError> {
        ...
        let collection = match self.database.collection().load_collection_by_user_and_id(user, collection_id.clone())? {
        ...
        self.database.collection().delete_collection(user, collection_id.clone())?;
    ...
    fn add_images_to_collection(&self, user: &User, collection_id: CollectionId, image_ids: &[ImageId]) -> Result<(), AddImagesToCollectionError> {
        ...
        let accessible_images = self.database.image().get_images_by_image_ids(user, image_ids)?;
        ...
        let added_count = self.database.collection().add_images_to_collection(user, collection_id.clone(), &accessible_images.iter().map(|image| image.id.clone()).collect::<Vec<ImageId>>())?;
    ...
    fn remove_images_from_collection(&self, user: &User, collection_id: CollectionId, image_ids: &[ImageId]) -> Result<(), RemoveImagesFromCollectionError> {
        ...
        let removed_count = self.database.collection().remove_images_from_collection(user, collection_id.clone(), image_ids)?;

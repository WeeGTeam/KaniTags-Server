...
impl TagService for TagServiceImpl {
    async fn get_tags(&self) -> Result<Vec<Tag>, GetTagsError> {
        ...
        let tags = self.database.tag().get_all_tags().await?;
    ...
    async fn get_image_tags(&self, image_id: ImageId) -> Result<Vec<ImageTag>, GetImageTagsError> {
        ...
        let image_tags = self.database.tag().get_image_tags_of_image(image_id).await?;
    ...
    async fn add_image_tags(&self, image_id: ImageId, new_tags: Vec<NewTag>, user: User) -> Result<Vec<ImageTag>, AddImageTagsError> {
        ...
        let tags = self.database.tag().get_tags_create_if_missing(new_tags).await?;
        let added_image_tags_number = self.database.tag().add_image_tags_to_image_by_user(tags, image_id, user).await?;
        info!("Added {} image tags to image {}", added_image_tags_number, *image_id);

        Ok(self.database.tag().get_image_tags_of_image(image_id).await?)
    }
}
...
mod test {
    ...
    mod test_add_image_tags {
        ...
        #[tokio::test]
        async fn should_add_tags_for_existing_image() {
            ...
            mock_tag_database.expect_add_image_tags_to_image_by_user()
                .with(eq(tags.clone()), eq(image_id), eq(user.clone()))
            ...
            let result = tag_service.add_image_tags(image_id, vec![NewTag::stub()], user).await.unwrap();

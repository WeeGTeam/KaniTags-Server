...
fn main() {
    ...
    let target_dir = cargo_toml_dir.join("..").join("target").canonicalize().unwrap();
    let file_path = target_dir.join("openapi.yaml");
    println!("writing openapi spec to {:?}", file_path);
    fs::write(file_path, api.to_yaml().unwrap()).unwrap();
}
